...
# Remove user data directory on purge
if [ "$1" = "purge" ]; then
    # Remove data directory for all users
    if [ -d /home ]; then
        for user_home in /home/*; do
            if [ -d "$user_home/.local/share/moe.yzf.bcm-convertor" ]; then
                rm -rf "$user_home/.local/share/moe.yzf.bcm-convertor"
            fi
        done
    fi
    # Also check root user
    if [ -d "/root/.local/share/moe.yzf.bcm-convertor" ]; then
        rm -rf "/root/.local/share/moe.yzf.bcm-convertor"
    fi
fi
          ...
          if [[ -n "$(find "$src" -type f -print -quit 2>/dev/null)" ]]; then
            while IFS= read -r -d '' file; do
              if [[ -n "$file" ]]; then
                ...
                local dst_dir="%{buildroot}/$RPM_LIB_DIR/$res$rel_dir"
...
# Remove user data directory for all users
if [ -d /home ]; then
    for user_home in /home/*; do
        if [ -d "\$user_home/.local/share/moe.yzf.bcm-convertor" ]; then
            rm -rf "\$user_home/.local/share/moe.yzf.bcm-convertor"
