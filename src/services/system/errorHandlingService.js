...
const ERROR_CODE_DEFINITIONS = {
  ...
  ABOUT_OUTPUT_DIRECTORY_OPEN_FAILED: {
    title: '打开输出目录失败',
    text: '无法自动打开输出目录，请手动前往桌面查看',
    stage: 'postprocess',
    retryable: true
  },
  BUILD_LOG_OPEN_FAILED: {
    title: '打开打包日志失败',
...
const MESSAGE_ERROR_RULES = [
  ...
  {
    code: 'BUILDER_TOOLCHAIN_MISSING',
  ...
  {
    code: 'REMOTE_ICON_CONVERT_FAILED',
...
const sanitizeRawAlertDetail = (value) => {
  ...
  const lines = extractReadableLines(rawText)
    ...
    .filter(
      (line) =>
        line && line !== '[object Object]' && !line.startsWith('at ') && !line.startsWith('{') && !line.startsWith('}')
    )
...
const getErrorAlertAction = (normalizedError, options = {}) => {
  const code = normalizeErrorCode(normalizedError?.code)

  if (code === 'OUTPUT_DIRECTORY_OPEN_FAILED' || code === 'ABOUT_OUTPUT_DIRECTORY_OPEN_FAILED') {
    if (typeof options.onRetryOpenOutput === 'function') {
      return {
        buttonText: '重试打开',
        handler: options.onRetryOpenOutput
      }
    }
    return null
  }

  if (code === 'BUILD_LOG_OPEN_FAILED') {
    if (typeof options.onRetryOpenLog === 'function') {
      return {
        buttonText: '重试打开日志',
        handler: options.onRetryOpenLog
      }
    }
    return null
  }

  if (normalizedError?.logPath && typeof options.onOpenLog === 'function') {
    return {
      buttonText: '打开日志',
      handler: options.onOpenLog
    }
  }
...
export const createUserFacingError = ({ code, title, text, detail, stage, retryable, logPath, cause }) => {
...
export const showErrorAlert = async (error, options = {}) => {
  const { error: normalizedError, title, text } = getErrorAlertContent(error)
  const action = getErrorAlertAction(normalizedError, options)
  const result = await showAlert(title, text, {
    buttonText: '关闭',
    actionButtonText: action?.buttonText
  })

  if (result === 'action' && typeof action?.handler === 'function') {
